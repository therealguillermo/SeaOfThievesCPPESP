...
void cCheat::lightDraw(std::vector<AActor> &displayActors) {
    ...
    this->player_controller = LocalPlayer.GetPlayerController();
    this->CameraManager = player_controller.GetCameraManager();
    ...
    //last camera fox check
    if (SOT->localCamera.fov > 120 || SOT->localCamera.fov <= 30) {
        SOT->localCamera.fov = this->lastFov;
    }
    else {
        this->lastFov = SOT->localCamera.fov;
    }

    //check last position
    if (SOT->localCamera.position.x == 0 || SOT->localCamera.position.y == 0 || SOT->localCamera.position.z == 0) {
        SOT->localCamera.position = this->lastPosition;
    }
    else {
        this->lastPosition = SOT->localCamera.position;
    }

    //check last angles
    if (SOT->localCamera.angles.x == 0 || SOT->localCamera.angles.y == 0 || SOT->localCamera.angles.z == 0) {
        SOT->localCamera.angles = this->lastAngles;
    }
    else {
        this->lastAngles = SOT->localCamera.angles;
    }

 
    int size = displayActors.size();
    for (int i = 0; i < displayActors.size(); ++i) {

        auto actor = displayActors[i];
        ...
        auto name = getNameFromIDmem(id);
         ...
         if (name.find("BP_Skeleton") != std::string::npos && name.find("Pawn") != std::string::npos) {
            ...
            auto Fauna = *reinterpret_cast<AFauna*>(&actor);
            ...
            auto treasure = *reinterpret_cast<AItemProxy*>(&actor);
...
void cCheat::readData(std::vector<AActor> &displayActors) {
    displayActors.clear();
    ...
    if (!GNames) {
        address = Mem->FindSignature(baseModule, baseSize, (BYTE*)"\x48\x8B\x1D\x00\x00\x00\x00\x48\x85\xDB\x75\x00\xB9\x08\x04\x00\x00", (char*)"xxx????xxxx?xxxxx");
    ...
    if (!GObjects) {
        ...
        GObjects = Mem->Read<uintptr_t>(address + gobjectsoffset + 22);
    }
    ...
    for (int i = 0; i < actors.Length(); ++i) {
        ...
        AActor actor = *reinterpret_cast<AActor*>(&actors[i]);
        if (actor == player_controller.GetActor())
            continue;
                ...
                */
                ...
                for (int p = 0; p < players.Length(); ++p) {
                    ...
                    if (tempPlayers.PlayerName == SOT->localPlayer.name)
                        ...
                        tempPlayers.PlayerName = "Player " + std::to_string(p);
