...
void cDrawing::DrawScoreBoard(int x, int y, std::vector<Team> Teams) {
    ...
    for (int i = 0; i < Teams.size(); ++i) {
        ...
        String(Teams.at(i).teamName.c_str(), x + 20, y + 20 + 60 * i, D3DCOLOR_XRGB(Teams.at(i).color.r, Teams.at(i).color.g, Teams.at(i).color.b), true, "small");
        for (int j = 0; j < 4; ++j) {
            ...
            if (j < Teams.at(i).Players.size())
                String(Teams.at(i).Players.at(j).PlayerName.c_str(), x + 40, y + 2 + (i * 4 * 15) + 15 * j, D3DCOLOR_XRGB(Teams.at(i).color.r, Teams.at(i).color.g, Teams.at(i).color.b), false, "small");
        }
...
void cDirectX::Render(bool  active, bool &shouldRead) {
